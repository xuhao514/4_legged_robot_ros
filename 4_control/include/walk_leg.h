...
{
public:
    //(x0,y0) 行走曲线中心点 L:步长 H:步高  _t_floor:着地相时长 _tair_tfloor_scale:Tair与t_floor的比值 _start_tn_tfloor_scale:周期的起始时刻与t_floor的比值 _delayt_tfloor_scale：delay_t与t_floor的比值
    void init(LegClass &_leg,float _x0,float _y0,float _L,float _H0,float _t_floor,float _tair_tfloor_scale,float _start_tn_tfloor_scale,float _delayt_tfloor_scale,int _bezierLen,int _forward);
    ...
    float get_start_tn_tfloor_scale(){return start_tn_tfloor_scale;};
private:
